<?php  
    ...
    if(isset($_POST["enviar"])){
        ...
        if ($correoCliente == $_POST["correo"] || $correoDomiciliario == $_POST["correo"]){
            $enviado = true;
        }else{
            $enviado = false;
    				...
    				<div class="card-body">
    					<?php if ($enviado) { ?>						
    						<div class="alert alert-success alert-dismissible fade show"
    							role="alert">
    							<?php  
    							 echo "<script>alert('Datos enviados');window.location = 'index.php?pid=".base64_encode("presentacion/enviarCorreo.php"). "&correo=" . $_POST["correo"] . "';</script>";
    							?>
    						</div>
    					<?php } ?>    					
    					<form
